...
class Optimizer(ABC):
    ...
    def __init__(self, instructions, opts, unnamed_points, unnamed_lines,
                 unnamed_circles, segments, seg_colors):
    ...
    def preprocess(self):
        process_instr_iter = self.instructions if self.verbosity < 0 else tqdm(
            self.instructions, desc="Processing instructions...")
        ...
        self.segments = [(self.lookup_pt(a), self.lookup_pt(b))
    ...
    def lookup_pt(self, p, name=None):
        ...
        if isinstance(p.val, FuncInfo):
            ...
            if head == "__val__":
                ...
                return self.midp(*self.lookup_pts(args))
            elif head == "circumcenter":
                return self.circumcenter(*self.lookup_pts(args))
            elif head == "orthocenter":
                return self.orthocenter(*self.lookup_pts(args))
            elif head == "incenter":
                return self.incenter(*self.lookup_pts(args))
            elif head == "centroid":
                return self.centroid(*self.lookup_pts(args))
            elif head == "amidp-opp":
                return self.amidp_opp(*self.lookup_pts(args))
            elif head == "amidp-same":
                return self.amidp_same(*self.lookup_pts(args))
            elif head == "excenter":
                return self.excenter(*self.lookup_pts(args))
            elif head == "foot":
                X, l = args
                foot_p = Point(
                    FuncInfo("inter-ll",
                             [l, Line(FuncInfo("perp-at", [X, l]))]))
                return self.lookup_pt(foot_p)
            elif head == "harmonic-conj":
                return self.harmonic_l_conj(*self.lookup_pts(args))
            elif head == "origin":
                assert (len(args) == 1)
                circ = args[0]
                ...
                if name:
                    self.make_lc_intersect(name, self.radical_axis(cnf1, cnf2),
                                           cnf1)
                else:
                    # Use random name as point is unnamed, but we still want to force LC to intersect
                    rand_name = get_random_string(6)
                    self.make_lc_intersect(rand_name,
                                           self.radical_axis(cnf1, cnf2), cnf1)
                return self.inter_cc(cnf1, cnf2, root_select)
            elif head == "isogonal-conj":
                return self.isogonal_conj(*self.lookup_pts(args))
            elif head == "isotomic-conj":
                return self.isotomic_conj(*self.lookup_pts(args))
            elif head == "inverse":
                return self.inverse(*self.lookup_pts(args))
                ...
                return self.midp_from(*self.lookup_pts(args))
            elif head == "mixtilinear-incenter":
                return self.mixtilinear_incenter(*self.lookup_pts(args))

            else:
                raise NotImplementedError(
                    f"[lookup_pt] Unsupported head {head}")
        else:
    ...
    def sample(self, i):
        s_method = i.sampler
        s_args = i.args
        if s_method == "acute-iso-tri":
            self.sample_triangle(i.points, iso=s_args[0], acute=True)
        elif s_method == "acute-tri":
            self.sample_triangle(i.points, acute=True)
        elif s_method == "equi-tri":
            self.sample_triangle(i.points, equi=True)
        elif s_method == "iso-tri":
            self.sample_triangle(i.points, iso=s_args[0])
        elif s_method == "polygon":
            self.sample_polygon(i.points)
        elif s_method == "right-tri":
            self.sample_triangle(i.points, right=s_args[0])
        elif s_method == "triangle":
            self.sample_triangle(i.points)
        else:
            raise NotImplementedError(
                f"[sample] NYI: Sampling method {s_method}")
    ...
    def sample_polygon(self, ps):
        ...
        angle_zs = [
            self.mkvar(name=f"polygon_angle_zs_{i}", lo=-2.0, hi=2.0)
            for i in range(len(ps))
        ]
        multiplicand = ((len(ps) - 2) / len(ps)) * math.pi
        angles = [
            multiplicand + (math.pi / 3) * self.tanh(0.2 * az)
            for az in angle_zs
        ]

        scale_zs = [
            self.mkvar(name=f"polygon_scale_zs_{i}", lo=-2.0, hi=2.0)
            for i in range(len(ps))
        ]
        ...
        self.register_loss("polygon-angle-sum",
                           angle_sum - expected_angle_sum,
                           weight=1e-1)

        # First point shoudl equal the last point
        self.register_loss("polygon-first-eq-last",
                           self.dist(Ps[0], Ps[len(ps)]),
                           weight=1e-2)
    ...
    def sample_triangle_on_unit_circ(self, ps):
        ...
        theta_a = self.mk_normal_var(f"{nA}_unit_angle",
                                     mean=(2 * math.pi) * (1 / 3),
                                     std=(2 * math.pi) * (1 / 8))
        theta_b = self.mk_normal_var(f"{nB}_unit_angle",
                                     mean=(2 * math.pi) * (2 / 3),
                                     std=(2 * math.pi) * (1 / 8))
        theta_c = self.mk_normal_var(f"{nC}_unit_angle",
                                     mean=(2 * math.pi) * (3 / 3),
                                     std=(2 * math.pi) * (1 / 8))
    ...
    def compute(self, i):
        ...
        if isinstance(i.computation, Point):
            ...
            raise NotImplementedError(
                f"[compute] NYI: {c_method} not supported")
    ...
    def parameterize(self, i):
        ...
        elif p_method == "in-poly":
            self.parameterize_in_poly(p_name, p_args[1])
        elif p_method == "on-circ":
            self.parameterize_on_circ(p_name, p_args[1])
        elif p_method == "on-line":
            self.parameterize_on_line(p_name, p_args[1])
        elif p_method == "on-ray":
            self.parameterize_on_ray(p_name, p_args[1])
        elif p_method == "on-ray-opp":
            self.parameterize_on_ray_opp(p_name, p_args[1])
        elif p_method == "on-seg":
            self.parameterize_on_seg(p_name, p_args[1])
        elif p_method == "on-minor-arc":
            self.parameterize_on_minor_arc(p_name, p_args[1])
        elif p_method == "on-major-arc":
            self.parameterize_on_major_arc(p_name, p_args[1])
        elif p_method == "line":
            self.parameterize_line(p_name)
        elif p_method == "through-l":
            self.parameterize_line_through(p_name, p_args[1])
        elif p_method == "tangent-lc":
            self.parameterize_line_tangentC(p_name, p_args[1])
        elif p_method == "tangent-cc":
            self.parameterize_circ_tangentC(p_name, p_args[1])
        elif p_method == "tangent-cl":
            self.parameterize_circ_tangentL(p_name, p_args[1])
        elif p_method == "through-c":
            self.parameterize_circ_through(p_name, p_args[1])
        elif p_method == "circle":
            self.parameterize_circ(p_name)
        elif p_method == "origin":
            self.parameterize_circ_centered_at(p_name, p_args[1])
        elif p_method == "radius":
            self.parameterize_circ_with_radius(p_name, p_args[1])
        else:
            raise NotImplementedError(f"FIXME: Finish parameterize: {i}")
    ...
    def parameterize_line_tangentC(self, l, args):
        ...
        tmp_l = Line(
            FuncInfo("connecting", [Point(FuncInfo("origin", [c])), P1]))
    ...
    def parameterize_circ(self, c):
        ...
        circ_nf = CircleNF(center=O,
                           radius=self.mkvar(name=f"{c.val}_origin",
                                             lo=0.25,
                                             hi=3.0))
    ...
    def parameterize_circ_centered_at(self, c, ps):
        ...
        circ_nf = CircleNF(center=origin,
                           radius=self.mkvar(name=f"{c.val}_origin",
                                             lo=0.25,
                                             hi=3.0))
        return self.register_circ(c, circ_nf)
    ...
    def parameterize_circ_tangentC(self, c, args):
        ...
        interP = self.parameterize_on_circ(Point(f"tangency_point"), [circ2],
                                           save_name=False)
    ...
    def parameterize_circ_tangentL(self, c, args):
        [l] = args
        interP = self.parameterize_on_line(Point(f"tangency_point"), [l],
                                           save_name=False)
        interP = Point(FuncInfo('__val__', [interP]))
        O = self.parameterize_on_line(Point(f"origin"),
                                      [Line(FuncInfo("perp-at", [interP, l]))],
                                      save_name=False)
        C = CircleNF(center=O, radius=self.dist(O, interP.val.args[0]))
        return self.register_circ(c, C)
    ...
    def parameterize_on_seg(self, p, ps):
        ...
        # z = 0.9 * z
        self.register_loss(f"{p}_seg_regularization",
                           z,
                           weight=1e-4,
    ...
    def parameterize_on_line(self, p, p_args, save_name=True):
        ...
        # z = 0.2 * z
        self.register_loss(f"{p}_line_regularization",
                           z,
                           weight=1e-4,
                           requires_train=False)
        ...
        P2 = B + (B - A).smul(s)
        # self.segments.append((A, B))
        return self.register_pt(p,
                                P1 + (P2 - P1).smul(self.sigmoid(z)),
                                save_name=save_name)
    ...
    def parameterize_on_circ(self, p, p_args, save_name=True):
        ...
        X = self.get_point(x=O.x + r * self.cos(theta),
                           y=O.y + r * self.sin(theta))
    ...
    def parameterize_on_minor_arc(self, p, p_args):
        ...
        Px = O.x + (anchor.x - O.x) * self.cos(theta) - (anchor.y -
                                                         O.y) * self.sin(theta)
        Py = O.y + (anchor.x - O.x) * self.sin(theta) + (anchor.y -
                                                         O.y) * self.cos(theta)
    ...
    def addNDG(self, ndg):
        ...
        ndg_val = tf.reduce_max(
            vals
        )  # Note how we reduce MAX because we are trying to make non-zero
    ...
    def assertion_vals(self, pred, args):
        ...
        # elif pred == "between": return self.between_gap(*self.lookup_pts(args))
            ...
            diffs = [
                self.coll_phi(A, B, C)
                for A, B, C in itertools.combinations(coll_args, 3)
            ]
            ...
            return [
                self.eqangle6_diff(A, B, C, P, Q, R),
            ...
            diffs = [
                self.eqangle6_diff(A, B, D, A, C, D)
                for A, B, C, D in itertools.combinations(cycl_args, 4)
            ]
            ...
            X, Y, A, B = self.lookup_pts(args)
            return [
                self.max(self.const(0.0),
                         self.dist(A, B) - self.dist(X, Y))
            ]
        elif pred == "eq-n":
            ...
            return [self.eqangle8_diff(*self.lookup_pts(args))]
            # elif pred == "eqoangle":
            # A, B, C, P, Q, R = self.lookup_pts(args)
            # return [self.angle(A, B, C) - self.angle(P, Q, R)]
        elif pred == "eq-ratio":
            return [self.eqratio_diff(*self.lookup_pts(args))]
        elif pred == "foot":
            f, x, l = args
            ...
            return self.in_poly_phis(*self.lookup_pts(args))
        elif pred == "inter-ll":
            ...
            return [self.coll_phi(*self.lookup_pts(args))
                    ] + self.onray_gap(*self.lookup_pts(args))
        elif pred == "on-seg":
            return [self.coll_phi(*self.lookup_pts(args))
                    ] + self.between_gap(*self.lookup_pts(args))
        elif pred == "opp-sides":
            a, b, l = args
            A, B = self.lookup_pts([a, b])
            lnf = self.line2nf(l)
            X, Y = self.lnf2pp(lnf)
            return [
                self.max(self.const(0.0), self.side_score_prod(A, B, X, Y))
            ]
            ...
            rhs_2 = (r1 + r2)**2
            return [
                tf.reduce_min([self.abs(lhs - rhs_1),
                               self.abs(lhs - rhs_2)])
            ]
        elif pred == "tangent-lc":
            l, c = args
            inter_point = Point(
                FuncInfo("inter-lc", [l, c, Root("arbitrary", list())]))
            ...
            raise NotImplementedError(f"[assertion_vals] NYI: {pred}")
    ...
    def matrix_mul(self, mat, pt):
        pt1, pt2 = mat
        return self.get_point(self.inner_product(pt1, pt),
                              self.inner_product(pt2, pt))
    ...
    def rotate_clockwise_90(self, pt):
        return self.matrix_mul(
            (self.get_point(self.const(0.0), self.const(1.0)),
             self.get_point(self.const(-1.0), self.const(0.0))), pt)
    ...
    def between_gap(self, X, A, B):
        ...
        return [
            self.diff_signs(X.x - A1.x, X.x - B1.x),
            self.diff_signs(X.y - A1.y, X.y - B1.y)
        ]
    ...
    def onray_gap(self, X, A, B):
        ...
        return [
            self.diff_signs(X.x - A1.x, A1.x - B.x),
            self.diff_signs(X.y - A1.y, A1.y - B.y)
        ]
    ...
    def inter_ll(self, l1, l2):
        (n11,
         n12), r1 = l1  # TODO(jesse): ensure that this pattern matching works
        (n21, n22), r2 = l2

        def inter_ll_aux(n11, n12, r1, n21, n22, r2):
            numer = n11 * r2 * (n21**2 + n22**2) - r1 * (n11**2 * n21 +
                                                         n12**2 * n21)
            ...
            return tf.cond(tf.less(tf.math.abs(denom), 1e-4), on_bad, on_ok)
    ...
    def inter_pp_c(self, P1, P2, cnf):
        ...
        def on_nneg():
            ...
            def sgnstar(x):
                return self.cond(self.lt(x, self.const(0.0)),
                                 lambda: self.const(-1.0),
                                 lambda: self.const(1.0))

            Q1 = self.get_point(
                (D * dy + sgnstar(dy) * dx * self.sqrt(radicand)) / (dr**2),
    ...
    def make_lc_intersect(self, name, lnf, c):
        ...
        loss = self.cond(
            self.logical_or(self.lt(self.dist(O, Operp), 1e-6),
    ...
    def second_meet_pp_c(self, A, B, O):
        ...
        return self.cond(self.lt(self.sqdist(A, P1), self.sqdist(A, P2)),
                         lambda: P2, lambda: P1)
    ...
    def radical_axis_pts(self, cnf1, cnf2):
        ...
        p1 = self.cond(
            test, lambda: self.get_point(x=(C - B) / A, y=self.const(1.0)),
    ...
    def eqratio_diff(self, A, B, C, D, P, Q, R, S):
        # AB/CD = PQ/RS
        return self.sqrt(self.dist(A, B) * self.dist(R, S)) - self.sqrt(
            self.dist(P, Q) * self.dist(C, D))
    ...
    def mixtilinear_incenter(self, A, B, C):
        ta, tb, tc = self.angle(C, A, B), self.angle(A, B,
                                                     C), self.angle(B, C, A)
        return self.trilinear(A, B, C, (1 / 2) *
                              (1 + self.cos(ta) - self.cos(tb) - self.cos(tc)),
                              1, 1)
    ...
    def invert_or_zero(self, x):
        return self.cond(
            self.abs(x) < 1e-5, lambda: self.const(0.0),
            lambda: self.const(1) / x)

    def isogonal_conj(self, P, A, B, C):
        x, y, z = self.to_trilinear(P, A, B, C)
        return self.trilinear(A, B, C, self.invert_or_zero(x),
                              self.invert_or_zero(y), self.invert_or_zero(z))

    def isotomic_conj(self, P, A, B, C):
        a, b, c = self.side_lengths(A, B, C)
        x, y, z = self.to_trilinear(P, A, B, C)
        return self.trilinear(A, B, C, (a**2) * self.invert_or_zero(x),
                              (b**2) * self.invert_or_zero(y),
                              (c**2) * self.invert_or_zero(z))
    ...
    def harmonic_l_conj(self, X, A, B):
        ...
        L = A + self.rotate_counterclockwise(self.const(math.pi / 3),
                                             X - A).smul(0.5)
        M = self.midp(A, L)
    ...
    def in_poly_phis(self, X, *Ps):
        ...
        for i in range(n):
            ...
            phis.append(
                self.max(self.const(0.0), -self.side_score_prod(X, C, A, B)))
        return phis
    ...
    def line2twoPts(self, l):
        if isinstance(l.val, str):
            ...
            if pred == "connecting":
                ...
                return self.line2twoPts(Line(FuncInfo("perp-at",
                                                      [m_ab, l_ab])))
            elif pred == "mediator":
                A, B = self.lookup_pts(args)
                M = self.midp(A, B)
                ...
                refl_p1 = Point(
                    FuncInfo("reflect-pl",
                             [Point(FuncInfo("__val__", [p1])), l2]))
                refl_p1 = self.lookup_pt(refl_p1)
                refl_p2 = Point(
                    FuncInfo("reflect-pl",
                             [Point(FuncInfo("__val__", [p2])), l2]))
                refl_p2 = self.lookup_pt(refl_p2)
                return refl_p1, refl_p2
            else:
                raise RuntimeError(
                    f"[line2twoPts] Unexpected line pred: {pred}")
        else:
        ...
        def mysterious_pp2pp(p1, p2):
            ...
            def pred(x, y):
                return tf.logical_or(
                    tf.math.less(y, self.const(0.0)),
                    tf.logical_and(tf.equal(y, self.const(0.0)),
                                   tf.math.less(x, self.const(0.0))))

            return tf.compat.v1.cond(pred(x, y), lambda: (p1, p2.smul(-1.0)),
                                     lambda: (p1, p2))

        def pp2lnf_core(p1, p2):
            p1, p2 = mysterious_pp2pp(p1, p2)
            x, _ = p2
            n = tf.compat.v1.cond(tf.less_equal(x, 0.0),
                                  lambda: self.rotate_clockwise_90(p2),
                                  lambda: self.rotate_counterclockwise_90(p2))
    ...
    def pp2sf(self, p1, p2):
        ...
        return self.cond(
            self.eq(p1.x, p2.x), vert_line, lambda: self.cond(
                self.eq(p1.y, p2.y), horiz_line, calc_sf_from_slope_intercept))
    ...
    def circ2nf(self, circ):
        ...
        if isinstance(circ.val, str):
            ...
            if pred == "c3" or pred == "circumcircle":
                ...
                return CircleNF(center=I,
                                radius=self.mixtilinear_inradius(A, B, C))
            else:
    ...
    def process_rs(self, P1, P2, root_select):
        ...
        if pred == "neq":
            ...
            [l] = rs_args
            P1_foot = Point(
                FuncInfo("foot", [Point(FuncInfo("__val__", [P1])), l]))
            P1_foot = self.lookup_pt(P1_foot)
            P2_foot = Point(
                FuncInfo("foot", [Point(FuncInfo("__val__", [P2])), l]))
        ...
        # return self.cond(test, lambda: P1, lambda: P2)
            ...
            return self.cond(self.opp_sides(P1, pt, a, b), lambda: P1,
                             lambda: P2)
        elif pred == "same-side":
            [pt] = self.lookup_pts([rs_args[0]])
            a, b = self.lnf2pp(self.line2nf(rs_args[1]))
            return self.cond(self.same_side(P1, pt, a, b), lambda: P1,
                             lambda: P2)
        elif pred == "arbitrary":
            return P2
        else:
