...
class Compute:
    ...
    def __str__(self):
        if isinstance(
                self.computation,
                collections.abc.Iterable) and type(self.computation) != str:
        ...
        return "define {obj} ({computation_str})".format(
            obj=self.obj_name, computation_str=comp_str)
...
class Parameterize:
    ...
    def __str__(self):
        ...
        elif isinstance(self.parameterization,
                        collections.abc.Iterable) and type(
        ...
        return "parameterize {p} {p_str}".format(p=self.obj_name,
                                                 p_str=param_str)
