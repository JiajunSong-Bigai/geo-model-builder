...
def build_aux(opts,
    ...
    with g.as_default():
        ...
        solver = TfOptimizer(instructions, opts, reader.unnamed_points,
                             reader.unnamed_lines, reader.unnamed_circles,
    ...
    for i, m in enumerate(filtered_models):
        ...
        if not (encode_fig or show_plot or save_plot):
            ...
            figs.append(
                m.plot(show=show_plot,
                       save=save_plot,
                       fname=f"{outf_prefix}_{i}.png",
                       return_fig=encode_fig,
                       show_unnamed=opts['unnamed_objects']))
    return figs
...
def build(opts,
    ...
    if problem_given:
        ...
        return build_aux(opts,
                         show_plot=show_plot,
                         save_plot=save_plot,
                         outf_prefix=outf_prefix,
                         encode_fig=encode_fig)
    else:
        dir_files = [
            f for f in listdir(opts['dir'])
            if isfile(join(opts['dir'], f)) and f[-1] != "~"
        ]
        ...
        if opts['experiment']:
            ...
            for _ in tqdm(range(n_trials), desc="Trials"):
                ...
                for f in tqdm(dir_files, desc="Problems"):
                    ...
                    if models and len(models[0].goals) > 0:
                        ...
                        trial_goal_success.append(
                            len([
                                v for v in model.goals.values()
                                if v <= opts['eps'] * 10
                            ]))
                    else:
            ...
            all_n_models_per_file = [
                mean(t_data) for t_data in all_trial_data
            ...
            all_n_models_with_gt0_models = [
                len([n for n in t_data if n > 0]) / len(dir_files) * 100
                for t_data in all_trial_data
            ]
            ...
            all_avg_success_times = [
                mean(success_time_data)
                for success_time_data in all_trial_success_times
            ]
            avg_success_time = mean(all_avg_success_times)
            std_success_time = stdev(all_avg_success_times)

            all_avg_fail_times = [
                mean(fail_time_data) for fail_time_data in all_trial_fail_times
            ]
            avg_fail_time = mean(all_avg_fail_times)
            std_fail_time = stdev(all_avg_fail_times)

            print(
                f"Models per File: Avg {avg_models_per_file}, Sd {std_models_per_file}"
            )
            print(
                f"% Problems with atleast 1 Model: Avg {avg_perc_success}, Sd {std_perc_success}"
            )
            print(f"Avg Time per Problem (all): Avg {avg_time}, Sd {std_time}")
            print(
                f"Avg Time per Problem (success): Avg {avg_success_time}, Sd {std_success_time}"
            )
            print(
                f"Avg Time per Problem (fail): Avg {avg_fail_time}, Sd {std_fail_time}"
            )

            # handle goals
            perc_goals_satisfied = list()
            for (trial_totals, trial_successes) in zip(all_trial_goal_total,
                                                       all_trial_goal_success):
                ...
                n_successful_problems_goals_satisfied = len([
                    n_goals for (n_total,
                                 n_goals) in zip(trial_totals, trial_successes)
                    if n_total == n_goals
                ])

                perc_goals_satisfied.append(
                    n_successful_problems_goals_satisfied /
                    n_successful_problems)

            print(f"Goal total info: {all_trial_goal_total}")
            print(f"Goal success info: {all_trial_goal_success}")
            print(
                f"% Successful Problems with Goals Satisfied up to Tolerance: Avg {mean(perc_goals_satisfied)}, Sd {stdev(perc_goals_satisfied)}"
            )
            pdb.set_trace()
