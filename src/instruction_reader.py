...
class InstructionReader:
    ...
    def register_circ(self, c):
        if c in self.circles:
            raise RuntimeError(
                f"[register_circ] Same circle declared twice: {c}")
    ...
    def register_line(self, l):
        if l in self.lines:
            raise RuntimeError(
                f"[register_line] Same line declared twice: {l}")
    ...
    def process_command(self, cmd):
        ...
        if head == "assert":
            ...
            raise NotImplementedError(
                f"[InstructionReader.process_command] Command not supported: {head}"
            )
    ...
    def process_param_circ(self, param):
        ...
        if pred == "origin":
            ...
            raise NotImplementedError(
                f"[process_param_circ] unrecognized param {param}")
        return pred, args
    ...
    def process_param_line(self, param):
        ...
        if pred == "through":
            ...
            raise NotImplementedError(
                f"[process_param_line] unrecognized param {param}")
        return pred, args
    ...
    def process_param_point(self, param):
        ...
        if pred == "on-seg":
            ...
            raise NotImplementedError(
                f"[process_param_point] unrecognized param {param}")
        return pred, args
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        raise RuntimeError(
                            f"Term {term} not a point/line/circle")
    ...
    def process_rs(self, rs_info):
        ...
        if rs_pred == "rs-neq":
            ...
            raise NotImplementedError(
                f"[process_rs] Unsupported rs pred: {rs_pred}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=
        'Arguments for compiling a file of instructions to an instruction set')
    parser.add_argument(
        '--problem',
        '-p',
        action='store',
